...
@just-web/commands@7.4.0 depends on `type-plus@8.0.0-beta.8`. This moves that runtime dependency to `8.0.0-beta.10`, which is a breaking change between prereleases: `Equal` takes an options object instead of positional `Then`/`Else`, `isType.f` is gone, `NonUndefined` was removed, and several `$`-prefixed symbols were renamed.
