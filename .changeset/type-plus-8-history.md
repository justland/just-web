...
`type-plus` is a development-only dependency here and does not appear in the emitted `.d.ts`, so @just-web/history@2.2.3 consumers neither resolve it nor inherit its `typescript >= 5.6.0` peer. Nothing about the published package changes; this keeps the repo on one `type-plus` version.
